<template>
  <div :style="panelStyles" class=" z-10 relative overflow-x-hidden overflow-y-auto bg-rose-200">
    ...
    <div v-if="posts.length" class="flex w-full flex-wrap justify-start gap-4 p-6">
      <article v-for="post in displayedPosts" :key="post.id" :style="randomPostStyle()"
        ...
        <img :src="base_url + post.image.url" :alt="post.image.alternativeText"
          class="transition-transform duration-500 group-hover:scale-110" :style="imageStyle" />
        <div class="p-4 flex flex-col ">
          <h3 class="text-xl sm:text-2xl font-semibold text-gray-900  group-hover:text-cyan-600 transition-colors">
            {{ post.title }}
          </h3>
          ...
          <NuxtLink :to="'/post/' + post.documentId"
            class="inline-flex items-center text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
            Подробнее
